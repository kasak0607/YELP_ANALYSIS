--1.find no of businesses in each cetegory
with cte as(select 
business_id, 
...
--  Find the most popular cetegories of businesses(based on the no. of reviews)
with cte as (
...
-- Find the top 3 most recent review for each business
with cte as (
    select r.*, b.name,
           row_number() over (
...
select b.business_id,
       ...
       sum(case when r.review_stars = 5 then 1 else 0 end) as stars_reviews,
       stars_reviews * 100 / total_reviews as percent_5_star
from tbl_yelp_reviews r
inner join tbl_yelp_business b
    on r.business_id = b.business_id
group by 1, 2
...
--8.Find the average rating of businesses that have at atleast 100 reviews
